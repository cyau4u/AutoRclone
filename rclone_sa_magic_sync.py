...
CNT_SA_EXIT = 20  # if continually switch account for 3 times stop script
...
def main():
    ...
    while id <= end_id + 1:
        ...
        # =================cmd to run=================
        rclone_cmd = "rclone --config {} sync --delete-before --drive-use-trash=false ".format(config_file)
        if args.dry_run:
            rclone_cmd += "--dry-run "
